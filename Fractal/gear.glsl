...
//vec3 DrawGear(vec3 color, vec2 uv, Gear g, float eps)
//{
...
//        DrawGear(color, uv+0.4*vec2(cos(t),sin(t)), inner, eps);
        ...
        d = smoothstep(eps, -eps, GearDe(uv+0.4*vec2(cos(t),sin(t)), inner));
        s = 1.0 - 0.7*GearShadow(uv+0.4*vec2(cos(t),sin(t)), inner);
        color = mix(s*color, inner.color, d);
    }

    // Draw outer gear:
    // DrawGear(color, uv, outer, eps);

    d = smoothstep(eps, -eps, GearDe(uv, outer));
    s = 1.0 - 0.7*GearShadow(uv, outer);
    color = mix(s*color, outer.color, d);
